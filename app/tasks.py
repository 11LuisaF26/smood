...
from app.models import escucha_credencial
...
class facebook:
    def __init__(self, id_red_social):
...
class twitter:
    ...
    @background(schedule=5)
    def obtener_twitters_user(nombre_usuario):
        data_redes = data_red.objects.all().values()
        list_publication_ids = []
        for data in data_redes:
            list_publication_ids.append(data["publicacion_id"])
    ...
    @background(schedule=5)
    def obtener_twitters_query(query):
        data_redes = data_red.objects.all().values()
        list_publication_ids = []
        for data in data_redes:
            list_publication_ids.append(data["publicacion_id"])
...
class instagram:
    ...
    @background(schedule=5)
    def get_user_medias(nombre_pagina, numero_paginas):
        credenciales = escucha_credencial.objects.all().values()
        instagram_username = credenciales["instagram_username"]
        instagram_password = credenciales[""]
        path = credenciales[""]
        instagram.with_credentials('victord.castanedap', 'testdemo', 'path/to/cache/folder')
        instagram.login()
        medias = instagram.get_medias('amazon', 1)

        for media in medias:
            print(media.__dict__.keys())
        data_redes = data_red.objects.all().values()
        list_publication_ids = []
        for data in data_redes:
            list_publication_ids.append(data["publicacion_id"])
